...
* <Item optional s={FontIo.FontIoConfig.encoding}/>:
    ...
    * <Member optional s={FontIo.FontIoConfig.encoding.forceCmapSubtableFormatToBePresent} type={boolean} />
        ...
        Whether to force a `cmap` format 4 subtable to be present even if it is empty or non-buildable (i.e. when all characters are mapped to exactly the same glyph). Under those situations, a “stub” `cmap` format 4 subtable containing no mapping will be created.
