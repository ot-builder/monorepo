...
* <Item optional s={FontIo.FontIoConfig.layout}/>:
    ...
    * <Member optional s={FontIo.FontIoConfig.layout.lookupWriteTricks} type={map(Ot.GsubGpos.LookupProp, number)} />
        ...
        Specific options to control writing behavior when writing `GSUB` and `GPOS` lookups, to provide platform-specific optimizations. The number is a flag-field and could be assigned as:
        ...
        * `0x0004`: Force single substitution and single position lookups to use flat (format 1) coverage table;
